...
"${PYTHON_BIN}" "${SCRIPT_DIR}/analyze_supervisor_feedback_behaviour.py" \
...
"${PYTHON_BIN}" "${SCRIPT_DIR}/analyze_supervisor_feedback_cost_feasibility.py" \
  --matrix-audit "${MATRIX_AUDIT}" \
...
"${PYTHON_BIN}" - \
  ...
  "${SCRIPT_DIR}/analyze_supervisor_feedback_behaviour.py" \
  "${SCRIPT_DIR}/analyze_supervisor_feedback_cost_feasibility.py" \
  "${BASH_SOURCE[0]}" \
  "${MATRIX_AUDIT}" <<'PY'
import hashlib
import json
...
expected_behaviour_sources = {
    "core/scripts/models/analyze_supervisor_feedback_behaviour.py": digest(behaviour_source),
    "core/scripts/models/run_supervisor_feedback_r3_offline_audits.sh": digest(runner_source),
...
payload = {
    ...
    "source_sha256": {
        "core/scripts/models/analyze_supervisor_feedback_behaviour.py": digest(behaviour_source),
        "core/scripts/models/analyze_supervisor_feedback_cost_feasibility.py": digest(cost_source),
        "core/scripts/models/run_supervisor_feedback_r3_offline_audits.sh": digest(runner_source),
