...
python data/prepare_prediction_dataset_split.py \
  --result_dir "${RESULT_DIR}" \
  --output_dir "${MERGED_DIR}"

python data/evaluate_prediction_dataset.py \
  --merged_dir "${MERGED_DIR}" \
  --split test \
  --output_json "${MERGED_DIR}/logged_baseline_metrics_test.json"

python training/finetune_multipath_carla.py \
  --merged_dir "${MERGED_DIR}" \
...
python training/evaluate_multipath_model_on_dataset.py \
  --merged_dir "${MERGED_DIR}" \
  --split test \
