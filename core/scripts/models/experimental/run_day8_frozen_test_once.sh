...
"${PYTHON_BIN}" "${SCRIPT_DIR}/experimental/freeze_day8_model_selection.py" \
...
"${PYTHON_BIN}" "${SCRIPT_DIR}/experimental/summarize_day8_frozen_test.py" \
  --results-dir "${DAY8_RESULTS}" \
  --test-dir "${TEST_DIR}" \
...
"${PYTHON_BIN}" "${SCRIPT_DIR}/experimental/package_day8_test_snapshot.py" \
  --test-dir "${TEST_DIR}" \
  --output "${SNAPSHOT}"
