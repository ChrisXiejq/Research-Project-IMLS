...
while true; do
  ...
  available_bytes="$(df -B1 --output=avail ${EXPERIMENT_STORAGE_ROOT:-/path/to/persistent-storage} | tail -1 | tr -d ' ')"
...
"$python_bin" "$script_dir/experimental/audit_thesis_core_v3_training.py" \
  --manifest "$manifest" \
...
"$python_bin" "$script_dir/experimental/prepare_future_mask_v4e_extension.py" \
  --manifest "$manifest" \
...
cd "$script_dir"
screen -dmS "$destination_screen" bash "$script_dir/experimental/run_future_mask_v4e_pipeline.sh" \
  "$destination_root" "$old_root" "$destination_root/feature_cache_v4" "$extension_protocol"

pipeline_pid=""
for _ in $(seq 1 30); do
  pipeline_pid="$(ps -eo pid=,comm=,args= | awk -v needle="$script_dir/experimental/run_future_mask_v4e_pipeline.sh $destination_root $old_root $destination_root/feature_cache_v4 $extension_protocol" '$2 == "bash" && index($0, needle) > 0 {print $1; exit}')"
...
"$python_bin" "$script_dir/experimental/write_future_mask_v4e_pipeline_receipt.py" \
  --pid "$pipeline_pid" \
...
screen -dmS "$destination_finalizer_screen" bash "$script_dir/experimental/finalize_future_mask_v4_offline.sh" \
