...
def check_existing_complete(root: Path, python_bin: str) -> bool:
    ...
    run_command(
        [
            python_bin,
            str(MODELS_DIR / "tools" / "package_closed_loop_snapshot.py"),
            "--verify-only",
            "--output",
            str(snapshot),
        ]
...
def main() -> None:
    ...
    run_command(
        [
            args.python_bin,
            str(PROCESS_DIR / "audit_r3_corrected_matrix.py"),
            "--results-dir",
            str(root),
            "--contract-json",
            str(contract_path),
    ...
    run_command(
        [
            args.python_bin,
            str(PROCESS_DIR / "analyze_r3_corrected_formal.py"),
            "--results-dir",
            str(root),
            "--contract-json",
            str(contract_path),
