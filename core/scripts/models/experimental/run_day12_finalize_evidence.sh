...
"$PYTHON_BIN" "$REPO_ROOT/core/scripts/models/experimental/audit_day6_collision_windows.py" \
  --day6-results "$DAY6_RESULTS" \
  --output-dir "$DAY12_RESULTS/collision_attribution"

echo "[$(date -Iseconds)] Day10 init-cluster analysis v3"
"$PYTHON_BIN" "$REPO_ROOT/core/scripts/models/experimental/analyze_day10_closed_loop.py" \
  --results-dir "$DAY10_RESULTS" \
  --output-dir "$DAY12_RESULTS/day10_analysis_v3"

echo "[$(date -Iseconds)] Day10+Day11 timing synthesis"
"$PYTHON_BIN" "$REPO_ROOT/core/scripts/models/experimental/analyze_day12_timing_synthesis.py" \
...
"$PYTHON_BIN" "$REPO_ROOT/core/scripts/models/experimental/package_day12_critical_assets.py" \
  --day6-results "$DAY6_RESULTS" \
  ...
  --anchors "$REPO_ROOT/core/scripts/models/assets/l5kit_clusters_16.npy" \
  --day10-results "$DAY10_RESULTS" \
  --day11-results "$DAY11_RESULTS" \
