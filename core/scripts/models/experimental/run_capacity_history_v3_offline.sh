...
MERGED_DIR="${MERGED_DIR}" BASE_MODEL="${BASE_MODEL}" ANCHORS="${ANCHORS}" \
  ...
  bash "${SCRIPT_DIR}/experimental/run_capacity_history_v3_training.sh" execute
...
"${PYTHON_BIN}" "${SCRIPT_DIR}/experimental/capacity_study_v3_pipeline.py" select \
...
"${PYTHON_BIN}" "${SCRIPT_DIR}/experimental/capacity_study_v3_pipeline.py" extensions \
...
"${PYTHON_BIN}" "${SCRIPT_DIR}/experimental/capacity_study_v3_pipeline.py" calibrate \
...
"${PYTHON_BIN}" "${SCRIPT_DIR}/experimental/capacity_study_v3_pipeline.py" freeze \
  --selection "${FINAL_SELECTION}" --fraction-selection "${FRACTION_SELECTION}" \
...
"${PYTHON_BIN}" "${SCRIPT_DIR}/experimental/capacity_study_v3_pipeline.py" fresh-evaluate \
...
"${PYTHON_BIN}" "${SCRIPT_DIR}/experimental/capacity_study_v3_synthesis.py" \
