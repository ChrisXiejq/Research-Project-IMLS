...
case "${MODE}" in
  prepare)
    "${PYTHON_BIN}" "${SCRIPT_DIR}/prepare_thesis_core_v3_dataset.py" \
      --source-dir "${SOURCE_DATASET}" --output-dir "${DATASET_DIR}"
    "${PYTHON_BIN}" "${SCRIPT_DIR}/build_thesis_core_feature_cache_v3.py" \
  ...
  plan|execute|audit)
    if [[ ! -s "${MANIFEST}" ]]; then
      ...
      "${PYTHON_BIN}" "${SCRIPT_DIR}/thesis_core_v3_runs.py" --output "${candidate}"
    ...
    "${PYTHON_BIN}" "${SCRIPT_DIR}/thesis_core_v3_execute.py" "${args[@]}"
    ;;
  *)
