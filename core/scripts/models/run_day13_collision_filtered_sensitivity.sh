...
"$PYTHON_BIN" "$SCRIPT_DIR/prepare_day13_collision_filtered_dataset.py" \
...
bash "$SCRIPT_DIR/run_day8_train_and_validate.sh"

echo "[$(date -Iseconds)] Compare original and filtered validation matrices"
"$PYTHON_BIN" "$SCRIPT_DIR/analyze_day13_filtered_sensitivity.py" \
