...
# Five-init precheck for the corrected give-way SMPC comparison.
# This is the low-risk step before 10-init or full 50-init expansion.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CORE_DIR="$(cd "${SCRIPT_DIR}/../.." && pwd)"
RESULTS_DIR="${RESULTS_DIR:-${CORE_DIR}/results/$(date +%Y%m%d_%H%M%S)_5init_final_dissertation}"
...
echo "5-init final dissertation precheck complete: ${RESULTS_DIR}"
