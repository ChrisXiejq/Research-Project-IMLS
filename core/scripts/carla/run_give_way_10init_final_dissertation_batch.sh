...
# Ten-init precheck for the phase-aware give-way SMPC comparison.
# This is the validation step between the passed 5-init precheck and full 50-init expansion.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CORE_DIR="$(cd "${SCRIPT_DIR}/../.." && pwd)"
RESULTS_DIR="${RESULTS_DIR:-${CORE_DIR}/results/$(date +%Y%m%d_%H%M%S)_10init_final_dissertation}"
...
TMP_INIT_DIR="${RESULTS_DIR}/_ego_init_01_10"
mkdir -p "${TMP_INIT_DIR}"
for idx_num in $(seq 1 10); do
...
echo "10-init final dissertation precheck complete: ${RESULTS_DIR}"
