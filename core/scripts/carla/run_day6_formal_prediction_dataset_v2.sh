...
MODEL_ANCHORS="l5kit_clusters_16.npy"
...
acquire_lock() {
  if mkdir "${LOCK_DIR}" 2>/dev/null; then
    ...
    if [[ "${old_pid}" =~ ^[0-9]+$ ]] && kill -0 "${old_pid}" 2>/dev/null; then
      ...
      if [[ "${old_command}" == *"run_day6_formal_prediction_dataset_v2.sh"* ]]; then
...
write_progress() {
  ...
  "${PYTHON_BIN}" "${MODELS_DIR}/summarize_prediction_dataset_v2_day6_progress.py" "${args[@]}"
}
...
"${PYTHON_BIN}" "${MODELS_DIR}/verify_prediction_dataset_v2_day6_preflight.py" \
  --repo-dir "${REPO_DIR}" \
...
bash "${SCRIPT_DIR}/run_give_way_prediction_dataset_v2.sh"

write_progress "collection_runner_returned" 0
AUDIT_JSON="${RESULTS_DIR}/day6_collection_audit.json"
"${PYTHON_BIN}" "${MODELS_DIR}/audit_prediction_dataset_v2_day6.py" \
  --results-dir "${RESULTS_DIR}" \
...
"${PYTHON_BIN}" "${MODELS_DIR}/finalize_prediction_dataset_v2_day6.py" \
  --results-dir "${RESULTS_DIR}" \
