...
"${PYTHON_BIN}" "${MODELS_DIR}/experimental/capacity_study_v3_collection.py" freeze \
...
PY
"${PYTHON_BIN}" "${MODELS_DIR}/experimental/capacity_study_v3_closed_loop.py" freeze \
...
PY
  ...
  "${PYTHON_BIN}" "${MODELS_DIR}/experimental/verify_capacity_history_v3_deployment.py" \
...
"${PYTHON_BIN}" "${MODELS_DIR}/experimental/capacity_study_v3_closed_loop.py" materialize-carla \
  --selection-freeze "${SELECTION_FREEZE}" --manifest "${MANIFEST}" \
  --results-dir "${RESULTS_DIR}" --output "${RESULTS_DIR}/MATERIALIZATION_AUDIT.json"
"${PYTHON_BIN}" "${MODELS_DIR}/experimental/capacity_study_v3_closed_loop.py" audit \
  --selection-freeze "${SELECTION_FREEZE}" --manifest "${MANIFEST}" \
...
PY
"${PYTHON_BIN}" "${MODELS_DIR}/experimental/capacity_study_v3_closed_loop.py" synthesize-carla \
  --selection-freeze "${SELECTION_FREEZE}" --manifest "${MANIFEST}" \
