...
if output.exists():
 ...
 if old_git!=new_git:
  ...
  allowed={"core/scripts/carla/experimental/run_day11_timing_shift_robustness.sh","core/scripts/models/experimental/audit_day10_closed_loop.py","core/scripts/models/tools/package_closed_loop_snapshot.py"}
...
"${PYTHON_BIN}" "${MODELS_DIR}/experimental/audit_day10_closed_loop.py" --results-dir "${DAY11_RESULTS}" --contract-json "${CONTRACT}" --output-json "${AUDIT}"
...
PY
"${PYTHON_BIN}" "${MODELS_DIR}/experimental/analyze_day11_timing_shift.py" \
  --results-dir "${DAY11_RESULTS}" --output-dir "${DAY11_RESULTS}/analysis"
"${PYTHON_BIN}" "${MODELS_DIR}/tools/package_closed_loop_snapshot.py" --results-dir "${DAY11_RESULTS}" --contract day11_run_contract.json --audit day11_closed_loop_audit.json --complete DAY11_COMPLETE.json --output "${DAY11_RESULTS}/day11_timing_shift_snapshot.tar.gz"
