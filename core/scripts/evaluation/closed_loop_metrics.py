...
class ClosedLoopTrajectory:
	...
	collision_probs  : Optional[np.ndarray] = None  # N, estimated chance-constraint violation probability
	...
	def __post_init__(self):
		...
		if self.collision_probs is not None and len(self.collision_probs.shape) > 1:
			raise ValueError(f"Collision probabilities should be a vector, but have shape: {self.collision_probs.shape}")
...
def get_average_collision_probability(cl_traj : ClosedLoopTrajectory) -> float:
	if cl_traj.collision_probs is None:
		return np.nan
	return np.nanmean(cl_traj.collision_probs)
...
class ScenarioResult:
	...
	def compute_metrics(self) -> dict:
		...
		metric_dict["average_collision_probability"] = get_average_collision_probability(self.ego_closed_loop_trajectory)
...
def load_scenario_result(pkl_path):
    ...
    # "static" vehicles in the scene should not be counted as target vehicles.
