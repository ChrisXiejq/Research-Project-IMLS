...
19. `Day13结果与论文数字Manifest.md`：Day13 结论、210 个稳定 result IDs、8 张 canonical tables 与引用规则；
20. `Day14论文写作证据包与Results草稿.md`：最终 RQs、实验设计、Results 英文草稿、Discussion 主线与 limitations；
21. `已完成实验与证据账本.md` 与 `generated/`：已完成证据。

实验数字、8 张表和 8 张主图已经冻结。论文正文只从 `generated/paper_assets_v1/` 引用数字与图表，不手工复制旧结果；下一步是将 Day14 写作包扩展成完整论文各章。
