...
完成状态（2026-08-02）：Day 9 已完成并通过。B1/B0 × fixed-medium/adaptive × assertive/reactive × train init01 共 8/8 个非正式 rollouts 均通过 deployment、prediction→risk→solver→supervisor 和 post-CARLA gates。共审计 1,761 个 prediction samples、1,737 个 debug steps；四个 reactive arms 各有 17 个 response-active samples；0 solver failure、0 invalid probability、0 invalid covariance。`DAY9_COMPLETE.json` 与 110-file compact snapshot 已在 `generated/day9/` 留档。该 smoke 只证明冻结部署与机制链可执行，不作为正式效果比较；Day 10 获准启动。执行与修复记录见 `Day9部署与CARLA_Smoke执行指南.md`。
...
10. Day 9 四个 reactive arms 均覆盖 response-active tail，且无预测数值或 solver failure；
