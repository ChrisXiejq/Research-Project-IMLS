...
状态（2026-08-01）：15/15 个训练与 validation 已完成，test 尚未访问。validation-only 排序选择 B1 / seed 37；T1 小幅优于 matched B2-M，但 T2 未优于 matched B2-D，两个 Transformer 均未超过 B1。下一步严格执行 pre-test freeze 和五个冻结代表模型的一次性 test；test 仅用于报告，不得改变 B1 / seed 37 的闭环选择。完整结果和命令见 `Day8完成报告与一次性Test执行指南.md`。
...
Day 1–7 已完成，Day 8 的 15/15 个 validation runs 已完成。当前唯一下一步是执行 Day 8 validation-frozen single test pass：

```text
B1/B2-M/B2-D/T1/T2 各自按 validation 冻结一个 representative seed
+ test 前锁定 closed-loop model = B1 seed 37
+ 五个代表模型各评价 test 一次
+ calibration 只允许加载 validation 参数
+ test 不能改变模型选择、触发调参或重训
+ 完成后生成 DAY8_COMPLETE.json 与 compact evidence snapshot
```

Day 8 validation 已确认：
...
6. `pre_response` 为零样本并按协议记为 `not_applicable`；
7. test 尚未访问，仍可执行无泄漏冻结；
8. Day 9 只允许部署 test 前冻结的 B1 / seed 37。
