...
| B1 | CARLA-finetuned MultiPath | 当前部署基线 | 必做 |
...
| T0 | 现有 repeated-current-context Transformer | 历史 pilot | 不重跑正式闭环 |
...
1. 实现 aligned ego-target history extraction；
2. 在 sample 中记录 interaction sequence 和 mask；
3. 实现 offline/online 共用 feature builder；
4. 实现 defensive-reactive target policy；
5. 记录 trigger/release/TTC/speed；
6. 新增 V2 collection script；
7. 增加 unit/static tests。
...
6. 实现 B2-M、B2-D、T1、T2；
7. 实现分组 residual heads；
8. 运行小样本 overfit 和 shape tests。
...
1. B2-M/B2-D/T1/T2 × seeds 11/23/37；
...
Day 2 已完成。下一步执行 Day 3：

```text
deployment-equivalent GMM evaluator
+ mixture NLL 与二维 covariance coverage
+ rollout-level aggregation
...
1. 旧数据无 init/source-subrun leakage；
2. V1 只能作为 deterministic negative-control pilot；
3. interaction pose sequence 可从 CARLA `AgentHistory` 对齐提取，velocity history 留到 Day 4 实现；
4. V2 四个 data cells 已展开为 200 个唯一 rollout IDs；
5. 同一 init 的四个 cells 被同一 grouped split 约束；
6. calibration evaluator 所需的 GMM raw output、probabilities、means、covariances、transforms 和 future labels 均已定位。
