...
```text
旧 prediction dataset split audit
+ V2 interaction sequence schema
+ 200-rollout collection manifest 冻结
```

Day 2 不启动正式数据采集，不训练新模型。它必须先回答：

1. 旧数据是否无 leakage；
2. interaction sequence 能否从 CARLA `AgentHistory` 稳定提取；
3. V2 四个 data cells 的配置如何程序化记录；
4. 新数据 merge 后如何保证同一 init 不跨 split；
5. calibration evaluator 需要哪些字段。
