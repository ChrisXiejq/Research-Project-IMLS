...
执行状态（2026-08-02）：代码与协议已实现，等待服务器运行。由于 Day 8 选择 B1/seed 37 且 T1/T2 未通过相对 B1 的 offline gate，Day 9 smoke 调整为 B1/B0 × fixed-medium/adaptive × assertive/reactive × train init01，共 8 个非正式 rollouts。新增 validation calibration 在线加载、model/calibration/anchors hash、GMM 数值、prediction→risk→solver→supervisor 链路和 response-active coverage 自动 gate。执行指南见 `Day9部署与CARLA_Smoke执行指南.md`。
...
Day 1–8 已完成。当前下一步是执行 Day 9 部署与 smoke：

```text
deploy frozen B1 seed 37
+ load validation-fitted calibration without refitting
+ verify normalization and model artifact hashes
+ run CARLA warm-up and inference smoke
+ audit prediction distribution -> risk controller logging chain
+ specifically inspect covariance scaling in response-active cases
+ smoke results do not enter the formal closed-loop matrix
```
...
9. Day 9 只允许部署 test 前冻结的 B1 / seed 37。
