...
Day 4 已完成。下一步执行 Day 5：

```text
init 01–05 × four-cell development smoke
+ reactive trigger/release coverage
+ S1/S0 future separation
+ minimum speed/acceleration/jerk/safety audit
+ freeze reactive parameters and collection config hash
```

Day 4 已确认：

1. V2 raster 的 OpenCV byte round-trip 与 ResNet preprocessing 在线/离线数值一致；
2. 6×12 interaction sequence 可由对齐的 ego/target pose+velocity history 统一生成；
3. V2 sample 保存 raw aligned states、sequence、mask、raster hash 和完整 treatment metadata；
4. S0/S1 × fixed/adaptive 四个单-init CARLA rollout 均成功并通过 dataset audit；
5. reactive target 在两个 S1 rollout 中均产生 2 次 trigger/release、44 个 active steps且未停车；
6. Day 4 实际减速仅约 1 m/s并存在二次 trigger，参数仍须 Day 5 在 init 01–05 上开发冻结；
7. `target_style`、`ego_policy` 和 `cell_id` 只用于审计/分层，禁止作为 predictor input。
