...
执行状态（2026-08-02）：已完成 16/80 个成功 rollouts；第 17 个因服务器侧 CARLA timeout 失败，未计入成功样本。续跑审计发现 v1 contract 对非确定性 GPU warm-up 浮点诊断做了整文件 hash，造成 false drift；v2 改为稳定部署语义 hash，并用一次性 provenance migration 保留旧 commit 的 16 个成功 rollouts。实验因素、模型、calibration、tuning、init 与 scenario 均未改变。
...
B0 fine-tuning bridge 已作为第二 predictor 完整整合进上述 80 rollouts，不再额外运行重复的 20 rollouts。只允许重跑技术失败，不允许改 threshold/model/risk/supervisor。
...
Day 1–9 已完成。当前下一步是执行 Day 10 A3 正式 closed-loop matrix：

```text
freeze and deploy B1 seed 37 plus B0 bridge
+ run 2 predictors × 4 risk policies × ID01–ID10
+ preserve A3 risk-owned-yield authority and frozen calibration
+ use rollout-level paired comparisons and fixed-risk frontier
...
11. Day 10 可启动，但必须继续把 B1 与 B0、adaptive 与完整 fixed frontier 的正式效果分开检验。
