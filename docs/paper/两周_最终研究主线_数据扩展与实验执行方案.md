...
Day 8 实际判定：T1/T2 均未超过 B1 的完整 offline gate，因此不得为了追求 Transformer 正结果而在 test 后选择其进入正式闭环。Day 10 部署 validation-only 选择的 B1，并以 B0 pretrained 作为模型侧对照；Transformer 改造保留为重要 negative/ablation evidence。
...
| --- | --- |
| B1 fine-tuned MultiPath / seed 37 | fixed aggressive |
| B1 | fixed medium |
| B1 | fixed conservative |
| B1 | adaptive floor_weak |
| B0 pretrained MultiPath | fixed aggressive |
| B0 | fixed medium |
| B0 | fixed conservative |
| B0 | adaptive floor_weak |
...
这 80 个 rollouts 是最终 closed-loop 主证据。不能为了节省时间只保留 fixed medium，因为原论文问题要求 adaptive 与 fixed frontier 公平比较。B1/B0 使模型侧因果对照覆盖同一个完整 risk frontier，不把模型效果与 risk-policy 选择混杂。

### 7.4 Fine-tuning bridge experiment

为了支撑论文从 pretrained MultiPath 到 CARLA fine-tuning 的叙事，B0 bridge 已整合进主 80-rollout matrix：

```text
B0 pretrained MultiPath
× fixed aggressive / fixed medium / fixed conservative / adaptive floor_weak
× 10 conditions
= 40 rollouts（与 B1 的 40 rollouts 完全配对）
```

这些结果与 B1 的四个 risk arms 直接配对，回答：

```text
CARLA fine-tuning 的 offline 改善是否改变 fixed/adaptive closed-loop behaviour？
```

Day 9 已证明 B0 可部署且无 invalid GMM，因此 B0 进入 Day 10。不会另加 20 个重复 bridge rollouts。
...
执行状态：已完成。200/200 rollouts、四个 cell 各 50、11,230 个 prediction samples，最终审计 `pass`。命令、断点续跑规则和成功标志见：

```text
docs/paper/Day6_正式200Rollout采集运行指南.md
...
执行状态：已完成。真实 200-rollout merge、grouped split、train-only normalization 和 TensorFlow model gate 全部 `pass`。得到 5,037 个可用 masked-label 样本（train/val/test = 4,036/506/495），五个模型分支均通过保存/加载与数值审计。执行记录和证据哈希见 `docs/paper/Day6完成与Day7执行指南.md`，紧凑结果见 `docs/paper/generated/day7/`。
...
状态（2026-08-02）：Day 8 已完整完成。15/15 个训练与 validation、25/25 个冻结 test subset 均通过，`DAY8_COMPLETE.json` 为 `pass`。Validation-only 选择 B1 / seed 37，独立 test 排名与 validation 完全一致；T1 优于 matched B2-M，但 T2 未优于 matched B2-D，两个 Transformer 均未超过 B1。Test 未参与模型选择，Day 9 部署模型保持 B1 / seed 37。完整结果见 `Day8最终Test结果与结论.md`。
...
完成状态（2026-08-02）：Day 9 已完成并通过。B1/B0 × fixed-medium/adaptive × assertive/reactive × train init01 共 8/8 个非正式 rollouts 均通过 deployment、prediction→risk→solver→supervisor 和 post-CARLA gates。共审计 1,761 个 prediction samples、1,737 个 debug steps；四个 reactive arms 各有 17 个 response-active samples；0 invalid probability、0 invalid covariance。每个 arm 的 solver failure fraction 为 0.47%–0.98%，低于冻结的 5% 门限且均由安全 fallback 处理。`DAY9_COMPLETE.json` 与 compact snapshot 已在 `generated/day9/` 留档。该 smoke 只证明冻结部署与机制链可执行，不作为正式效果比较；Day 10 获准启动。执行与修复记录见 `Day9部署与CARLA_Smoke执行指南.md`。
...
完成状态（2026-08-02）：16/16 cells、80/80 rollouts 与最终审计全部通过。0 collision、0 yield-order failure、0 completion failure、0 invalid probability/covariance；最大 solver failure fraction 1.00%，低于 5% gate。B1 跨策略平均没有优于 B0；fixed-aggressive 下出现较快 completion 的条件性信号，但 multiplicity 校正后未确认。Adaptive 位于 fixed safety–efficiency frontier，而不是普遍支配 frontier。完整配对结果见 `Day10正式闭环结果与结论.md`。

运行 80-rollout 主矩阵：

```text
2 predictors × 4 risk policies × 2 target styles × 5 held-out inits
```

B0 pretrained bridge 已作为第二 predictor 完整整合进上述 80 rollouts，不再额外运行重复的 20 rollouts。只允许重跑技术失败，不允许改 threshold/model/risk/supervisor。

### Day 11：Timing-shift robustness

完成状态（2026-08-02）：16/16 cells、80/80 rollouts 与审计均 PASS。B1 在 fixed-medium/adaptive 的 ±3 m 扰动下呈现 `-0.703/-0.558 s` 的局部效率信号，但 margin 较小且 solver/supervisor activity 增加；adaptive 未普遍优于 fixed-medium。正式结果见 `Day11时序偏移稳健性结果与最终论文定位.md`。
...
原计划的 v12 20-rollout subset 取消：它不会直接补强当前机器学习中心论点。Day12 改为：

1. 归因 Day6 六个 training-only target–infrastructure collision rollouts，并量化与可训练 window 的重叠；
2. 将 Day10 pooled inference 修正为 5-init cluster inference；
3. 合并 Day10 offset=0 与 Day11 offset=±3 m，生成三水平 timing synthesis；
4. 为 Day6 dataset、Day8 frozen models 和 Day10/11 snapshots 建立第二份已验证备份；
5. 生成最终机器可读证据 manifest。

详细完成门见 `Day11后论文级实验审计与Day12至Day14执行方案.md`。
...
Day1–Day11 已完成。当前不再新增或调参主体模型；下一步是 Day12 论文级统计修复、三水平 timing synthesis 和关键资产备份。Canonical 执行方案已迁移到：

```text
docs/paper/Day11后论文级实验审计与Day12至Day14执行方案.md
```

Day 8 已确认：

1. 五种模型 × 三个 seeds 共 15/15 runs 完成；
2. 所有模型的 invalid covariance 均为 0；
3. B1 三 seed 的主要指标和 reactive ADE 高度稳定；
4. validation 排名为 B1、B2-D、T2、T1、B2-M；
5. T1 优于 matched B2-M，但 T2 未优于 matched B2-D；
6. test 排名与 validation 排名完全一致，没有明显 ID generalization collapse；
7. validation `pre_response` 为零样本，test 则有 60 个样本，必须报告 split coverage 不均衡；
8. aggregate calibration 在 test 总体有效，但对 15-sample response-active tail 明显恶化；
9. Day 9 仅部署 test 前冻结的 B1 / seed 37，并已通过 8/8 deployment smoke；
10. Day 9 四个 reactive arms 均覆盖 response-active tail，无预测数值失败；各 arm solver failure fraction 为 0.47%–0.98%，由 fallback 安全处理；
11. Day 10 已完成：B1 与 B0、adaptive 与完整 fixed frontier 已按 held-out condition 配对分析；
12. Day 11 只复核冻结的 fixed-medium/adaptive timing shift transfer，不做结果驱动调参。
