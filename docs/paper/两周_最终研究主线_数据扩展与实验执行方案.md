...
Day 3 已完成。下一步执行 Day 4：

```text
V2 raster offline/online channel contract
+ aligned 6×12 ego-target interaction history
+ defensive-reactive target
+ four-cell single-init collection smoke
```

Day 3 已确认：

1. GMM raw-output decoder 在 evaluator/deployment 间数值等价；
2. mixture NLL、2D coverage、invalid covariance 和 rollout aggregation 可统一生成；
3. calibration 参数只能由 validation rollouts 拟合，test fitting 已被代码禁止；
4. 旧 B1/T0 训练 loader 与在线 CARLA raster channel 不一致；
5. T0 相对 B1 仍有 point-error 改善，但两者都不是正式 V2 证据；
6. validation-NLL-optimal global scaling 会恶化 marginal coverage，不能把 NLL 改善等同于校准成功；
7. 正式 B1 必须在 V2 shared raster contract 下重新 fine-tune。
