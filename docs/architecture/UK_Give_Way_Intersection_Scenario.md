...
| Rule-aware yielding state machine | not present | enabled, `yield_stop_speed=0.2m/s`, `yield_reference_min_speed=0.8m/s`, `yield_reference_decel=-4.0m/s^2`, `yield_stop_decel=-5.0m/s^2`, `yield_stop_buffer_distance=6.25m`, `yield_brake_distance_margin=3.0m` | Define a fixed route-level conflict zone and a physically reachable stop point before it, then move through cautious approach, hold, release, and recovery phases |
