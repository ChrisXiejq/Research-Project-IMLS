...
| Ego yield-stop supervisor | not present | enabled, `yield_stop_speed=0.2m/s`, `yield_stop_buffer_distance=10.0m` | Define a fixed route-level conflict zone and a stop point before it, so the turning ego waits before entering the shared conflict area |
...
The scenario configuration alone does **not** hard-code a traffic-light rule. The controller does include a safety supervisor for this unsignalised give-way experiment. The supervisor first defines a fixed conflict point from the ego global left-turn route and the priority target prediction, then places an ego stop point before that conflict point. While the straight-going target is predicted to occupy the conflict zone, the turning ego brakes toward this stop point and uses the route lookahead steering for the intended left turn, instead of continuing to drift through the junction. After the target clears the zone, a bounded post-yield recovery window lets the ego regenerate a local reference from its stopped pose and resume the turn.
...
- the route-defined yield-stop supervisor that preserves the right-of-way rule when the optimiser would otherwise keep moving through the turn,
