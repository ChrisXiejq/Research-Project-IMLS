...
The `ego_init_01` CARLA rollout now uses `init_speed=6.0m/s`. The previous `9.36m/s` initial speed placed the ego about `2.45m` before the stop point while requiring more than `14m` of braking distance, so even an immediately active cautious-yield controller could not stop before the yield line. The stop buffer is set to `6.0m`: this keeps the activation distance at `12.0m`, moves the stop point slightly closer to the conflict zone than the `6.5m` trial, and gives the optimiser a little more braking path after activation without making the ego react earlier to a distant target. The release condition also waits until the priority target has cleared the configured conflict radius, preventing recovery from starting while the target is still inside the conflict zone.
