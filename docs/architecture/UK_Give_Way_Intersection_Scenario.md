...
| Ego post-yield recovery | not present | enabled, `yield_recovery_speed=3.0m/s` as a speed cap | After the priority target clears the conflict zone, rebuild a low-speed rejoin reference and resume the turn instead of either remaining stuck near the yield point or accelerating away from the route |
...
Once the target prediction is valid, the state machine defines a fixed conflict point from the ego global left-turn route and the target's current/predicted straight motion line, then places an ego stop point before that conflict point. While the straight-going target is approaching or occupying the conflict zone, the turning ego enters `approach_yield_line` once its distance to the stop point is below `v^2 / (2 |a_min|) + margin`, then transitions to `hold_yield_line` near the stop point. During these phases it brakes toward the stop point, uses route lookahead steering for the intended left turn, and caps the pre-solve SMPC reference at the yield speed so the optimiser is not still chasing a high-speed path through the occupied conflict zone. After the target clears the zone, `released_recovery` caps the local reference at `yield_recovery_speed` and applies only bounded restart acceleration, allowing the ego to rejoin the left-turn route without the large speed spike seen in the `20260620_152122` rollout.
