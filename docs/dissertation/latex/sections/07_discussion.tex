...
that distribution into constraints, solver feasibility limits candidate
motion, and the supervisor can change execution. Because B1 and B0 also use
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}L{0.25\linewidth}L{0.31\linewidth}L{0.34\linewidth}@{}}
    ...
    \midrule
    Five independent init groups & Paired effects, cluster intervals and exact tests & Minimum two-sided exact $p=0.0625$ \\
    ...
    Coupled solver/supervisor & Corrected contracts and mechanism telemetry & No single-component attribution \\
    Zero observed binary failures & Continuous actual-bbox separation reported & No zero-risk or equivalence claim \\
    Very small active tail & Separate labelled legacy diagnostic & No stable tail-population estimate \\
...
\TODO{Tie each interpretive claim to checked primary literature and state
where the result agrees, conflicts, or adds a boundary.}
