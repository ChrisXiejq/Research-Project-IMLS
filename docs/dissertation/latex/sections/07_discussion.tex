...
The formal closed-loop experiment first establishes that the deployed
manipulation is real: B1 improves all
...
supervisor and physical margin---but are not pooled with the formal matrix and do not identify
...
\input{../../paper/generated/supervisor_feedback_v1/paper_integration/supervisor_feedback_final_discussion.tex}
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}L{0.25\linewidth}L{0.31\linewidth}L{0.34\linewidth}@{}}
    ...
    Five formal closed-loop init groups & Paired effects, cluster intervals and exact tests & Minimum two-sided exact $p=0.0625$ \\
    ...
    Predictor calibration differs & Stack-level estimand declared before the formal experiment & No weight-only B1 effect \\
    Coupled solver/supervisor & Prospectively specified corrected-supervisor application-authority on/off intervention & The DID covers adaptive versus the criticised fixed-medium pair only; estimator-to-risk path and SMPC constraints remain in both arms \\
    ...
    Legacy implementation differs & The formal closed-loop matrix is primary; no pooling & Legacy evidence is mechanism-only \\
    \bottomrule
  \end{tabular}
\end{table}
...
invalidate the frozen in-distribution and formal closed-loop comparisons.
